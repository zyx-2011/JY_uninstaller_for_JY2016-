...
static void getProcessName()
{
    ...
    /*————————————————
        ...
        原文链接：https ://blog.csdn.net/u014677109/article/details/127444425*/
}
static bool write_preventkill(char filename[]) {
    getProcessName();
    fstream file1;
...
begin_main_into:
    system("title 极域卸载工具    v2.8.2(控制台版)");
    fstream file;
    ...
    setColor(FOREGROUND_INTENSITY | FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
    cout << "极域卸载工具    Control v2.8.2      适用于:Windows 10 极域电子教室2016" << endl << endl;
    print_warn();
    ...
    print_info();
    cout << "开始释放文件……" << endl;
    if (write_preventkill(argv[0]) && write_msgvbs()) {
        print_info();
        cout << "释放成功" << endl;
    }
    else {
...
begin_first:
    ...
    while (1) {
        ...
        if (xx == 1) {
            ...
            cout << "开发工具：Visual Studio 2022" << endl << endl;
            cout << "---------------版本信息---------------" << endl;
            cout << "当前版本：2.8.2(控制台版)" << endl;
